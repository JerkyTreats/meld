...
pub fn add_to_builder(
    ...
    let env_name =
        std::env::var("MERKLE_ENV").unwrap_or_else(|_| "development".to_string());
    ...
    if base_config_path.exists() {
        builder = builder.add_source(
            File::with_name(base_config_path.to_str().unwrap()).required(false),
        );
    }
    ...
    if env_config_path.exists() {
        builder = builder.add_source(
            File::with_name(env_config_path.to_str().unwrap()).required(false),
        );
    }
