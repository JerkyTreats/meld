//! Context owner service for current selections, frame mutation, and publication.
...
pub struct ContextService {
    ...
    /// Context-owned durable current selections.
    head_index: parking_lot::RwLock<HeadIndex>,
...
impl ContextService {
    ...
    pub fn new(
        ...
        Self {
            node_store,
            frame_storage,
            head_index: parking_lot::RwLock::new(head_index),
    ...
    fn persist_indices(&self) -> Result<(), ApiError> {
        ...
        if let Some(append) = self.durable_event_append() {
            crate::context::publication::publish_heads(
                &append,
                &head_index,
                self.frame_storage.as_ref(),
                &self.context_write_session_id()?,
            )?;
        }
        Ok(())
    }
    ...
    /// Create basic node metadata for a writable Agent when its frame is absent.
    ...
    /// Inspect Context selections without exposing a mutation capability.
    pub fn heads(&self) -> parking_lot::RwLockReadGuard<'_, HeadIndex> {
        self.head_index.read()
    }
...
impl CurrentFrameHeadRead for ContextService {
    ...
    fn current_frame_heads_for_node(&self, node_id: &NodeID) -> Result<Vec<FrameID>, ApiError> {
        Ok(self.head_index.read().active_heads_for_node(node_id))
    }
    ...
    fn count_nodes_for_frame_type(&self, frame_type: &str) -> Result<usize, ApiError> {
        Ok(self
            .head_index
            .read()
            .count_nodes_for_frame_type(frame_type))
    }
}
...
mod tests {
    ...
    fn create_test_api() -> (ContextService, TempDir) {
    ...
    fn test_ensure_agent_frame_creates_when_missing() {
        ...
        let frame_id = api
            .ensure_agent_frame(node_id, "writer-1".to_string(), None)
            .unwrap();
    ...
    fn test_ensure_agent_frame_skips_reader_agents() {
        ...
        let frame_id = api
            .ensure_agent_frame(node_id, "reader-1".to_string(), None)
            .unwrap();
