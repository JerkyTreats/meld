...
use super::{
    ...
    SeedSourceSpec, TargetSelectorKind, TaskPackageSpec, TraversalPrerequisitePackageExpansionSpec,
...
pub fn prepare_workflow_package_context<E, A, R>(
    api: &A,
    workspace_root: &Path,
    ...
    resolve_prompt: R,
) -> Result<PreparedWorkflowPackageContext, E>
where
    E: From<ApiError>,
    ...
    R: FnMut(&str) -> Result<String, E>,
{
    ...
    let target_node_record = api
        .read_execution_node_record(&target_node_id)?
        .ok_or_else(|| missing_node(target_node_id))?;
    ...
    Ok(PreparedWorkflowPackageContext {
        target_node_id,
        target_path: target_node_record.path,
        prompts_by_turn_id,
        gates_by_id,
        traversal_expansion,
    })
...
pub fn validate_workflow_package_trigger<E>(
    package_spec: &TaskPackageSpec,
    registered_profile: &RegisteredWorkflowProfile,
    request: &WorkflowPackageTriggerRequest,
) -> Result<(), E>
...
pub fn find_traversal_prerequisite_expansion<E>(
    package_spec: &TaskPackageSpec,
) -> Result<&TraversalPrerequisitePackageExpansionSpec, E>
where
    E: From<ApiError>,
{
    package_spec
...
pub fn resolve_package_target_node_id<E, A>(
    api: &A,
    workspace_root: &Path,
    request: &WorkflowPackageTriggerRequest,
) -> Result<NodeId, E>
where
    A: NodeResolutionPort<Error = E, NodeId = NodeId> + ?Sized,
{
...
pub fn prompt_map<E, R>(
    turns: &[TurnSpec],
    mut resolve_prompt: R,
) -> Result<HashMap<String, String>, E>
where
    R: FnMut(&str) -> Result<String, E>,
{
    let mut prompts = HashMap::new();
    for turn in turns {
        let prompt = resolve_prompt(&turn.prompt_ref)?;
...
fn missing_runtime_field<E>(package_spec: &TaskPackageSpec, field: &str) -> E
where
    E: From<ApiError>,
{
    config_error(format!(
...
fn missing_node<E>(node_id: NodeId) -> E
where
    E: From<ApiError>,
{
    ApiError::GenerationFailed(format!("Package target node '{:?}' was not found", node_id)).into()
}
