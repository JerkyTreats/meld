//! Native lifecycle transitions shared by world-model runtime owners.
...
/// Native phase retained by one concrete world-model owner instance.
...
/// Stateful lifecycle seam owned by one world-model actor instance.
#[derive(Debug)]
pub struct NativeLifecycle {
    owner_ref: String,
    records: Mutex<BTreeMap<NativeLifecycleIdentity, NativeLifecycleRecord>>,
}
...
impl NativeLifecycle {
    ...
    fn transition(
        ...
        let proof_ref = format!(
            "world-model-native-transition::{}::{}::{}::{from:?}-to-{next:?}::{checkpoint_ref}",
...
mod tests {
    ...
    fn native_owner_requires_safe_point_before_stop_and_retains_transition_proofs() {
        let lifecycle = NativeLifecycle::new("belief.owner");
        let identity =
            NativeLifecycleIdentity::new("generation-1".into(), "incarnation-1".into()).unwrap();
        let start = lifecycle.start(identity.clone(), "belief::7").unwrap();
        assert_eq!(start.to, NativeLifecyclePhase::Running);
        assert!(lifecycle.stop(identity.clone(), "belief::7").is_err());
        let safe = lifecycle.safe_point(identity.clone(), "belief::7").unwrap();
        let stop = lifecycle.stop(identity.clone(), "belief::7").unwrap();
        let release = lifecycle.release(identity, "belief::7").unwrap();
    ...
    #[test]
    fn successor_incarnation_authors_fresh_readiness_and_conflicting_retry_fails() {
        let lifecycle = NativeLifecycle::new("graph.owner");
        let first =
            NativeLifecycleIdentity::new("generation-1".into(), "incarnation-1".into()).unwrap();
        let successor =
            NativeLifecycleIdentity::new("generation-1".into(), "incarnation-2".into()).unwrap();
        let first_proof = lifecycle.start(first.clone(), "graph::7").unwrap();
        assert_eq!(
            lifecycle.start(first.clone(), "graph::7").unwrap(),
            first_proof
        );
        assert!(lifecycle.start(first, "graph::8").is_err());
        let successor_proof = lifecycle.start(successor, "graph::7").unwrap();
        assert_ne!(first_proof.proof_ref, successor_proof.proof_ref);
        assert_eq!(successor_proof.incarnation_id, "incarnation-2");
    }
}
