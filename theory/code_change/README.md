...
Observation is currently scoped to an admission epoch. A delayed Task callback can recover the retained proposal and materialization without repeating a write, even after the proposal disappears or the user edits the files. If the process closes that epoch before confirmation, the prior Task return remains recoverable, but its materialization does not satisfy the new epoch's separate Goal. Durable request continuity across that transition remains implementation work. This package also does not yet connect automatic mitigation selection to the concurrently running Security Agent.
