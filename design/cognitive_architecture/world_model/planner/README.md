...
The planner-facing projection's downstream output is `WorldState` — a set of ground `Proposition` values in the shared language [`meld-lang`](../../meld-lang/README.md). This is the concrete contract at the world-model-to-execution seam.

The minimal projection projects directly from `BeliefView` plus graph scope into `WorldState` through `project_world_state` and `PlannerQuery::project_current_world_state`.

The broader planner architecture assembles `WorldModelView` from lower-layer inputs and translates that richer view into `WorldState` for execution consumption:
