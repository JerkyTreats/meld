...
The world model builds on a shared event spine, explicit graph attachment through `DomainObjectRef` and `EventRelation`, replayable graph materialization, current anchor selection, lineage, provenance, traversal indexes, and branch-scoped federation.

`world_model/graph` is the substrate consumed by other world model layers.
Belief, planner-facing projection, causation, and regime layers build on graph services without re-reading raw events or importing source-domain internals.
...
- `meld-world-model` is the authority crate for graph materialization and world-model compatibility surfaces.

Within the world model, graph is the durable materialization layer and belief is the inference layer.
Causation, regime, and planner-facing projection consume graph and belief services rather than reconstructing their own view from raw event replay.

## Multi-Agent Grounding

An agent is a runtime-owned set of directives, goals, perspectives, and curation policy over specific `DomainObjectRef` values.
