...
| `merkle_traversal` | consume | Merkle traversal is a capability and expansion source used by execution paths | complete | [merkle_traversal.rs](../../src/merkle_traversal.rs), [Task Network](execution/control/task_network.md) | none | Keep as agent-side capability input, not KG source truth |
...
| `task` | own | Task owns compiled task structure, task artifacts, events, runtime, and package behavior | complete | [task.rs](../../src/task.rs), [Execution Substrate](execution/substrate.md) | none | Keep task under agent execution and publish outcome evidence to spine |
