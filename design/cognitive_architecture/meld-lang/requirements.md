...
- Steps are either operators or sub-goals (recursive: the planning loop finds a composition for sub-goals).
- Edges carry one of: `Ordering` (must complete before), `DataFlow` (passes artifact of a given type), `Conditional` (activates only if a guard condition on a field of the upstream step's output holds).
- Compositions are constructed at runtime. They may be hand-authored, loaded from files, generated by LLM, or constructed programmatically by the world model agent or planning loop.
...
- Methods are optional. The system must function without methods by having the world model agent or planning loop construct compositions directly.
...
- `Indeterminate` is distinct from `Unsatisfied`. It means the world model has not asserted anything about the referenced dimension or object. The planning loop may react to indeterminate results differently (triggering observation rather than action).
...
- Validation does NOT check whether effects achieve a goal. That is the planning loop's responsibility.
