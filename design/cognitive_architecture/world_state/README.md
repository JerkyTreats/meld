...
The world model now has three visible strata:
...
The graph layer consumes canonical events through `DomainObjectRef` objects and `EventRelation` edges, materializes traversal indexes, selects current anchors, preserves anchor lineage, and emits durable derived anchor events back into the event ledger.
...
The durable world model is a temporal graph and belief system with:
...
- projection of event facts into current world-model state
...
`telemetry` is now a downstream compatibility and reporting surface over the event ledger.

## Code Name

The target crate name is `meld-world-model`.
The current Rust module remains `src/world_state` until compatibility migration is complete.
Use `world_state` only when referring to existing code paths or event domain identifiers.

## Current Contract

- canonical events carry graph objects and relations
- graph materialization is replayable from event history
...
- [Temporal Fact Graph](graph/temporal_fact_graph.md)
  implemented graph model for the graph layer and its event contract
...
- [World Model Crate](CRATE.md)
  crate boundary, owned modules, migration path, and forbidden dependencies

## First Slice Boundary

The graph slice is now part of the baseline architecture.

The remaining world-model design work starts above that baseline:
