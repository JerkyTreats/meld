...
### Phase 1: Add Provider Registry (Non-Breaking)

- Implement provider registry with XDG loading
- Support both old (config.toml) and new (XDG) provider locations
- Old agents with `provider_name` continue to work
- New agents can be provider-agnostic

### Phase 2: Update Agent Loading

- Remove `provider_name` requirement from agent config
- Remove `completion_options` from agent config
- Update agent loading to not embed providers
- Runtime provider selection becomes required

### Phase 3: CLI Updates

- Add `--provider` flag to context generation commands
- Update help text and examples
- Add provider management commands

### Phase 4: Deprecation

- Mark inline `provider_name` in agent config as deprecated
...
- [Agent Management Requirements](agent_management_requirements.md) - Agent configuration
- [Provider Management Requirements](provider_management_requirements.md) - Provider configuration (TBD)
