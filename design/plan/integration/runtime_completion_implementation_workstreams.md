...
> Historical delivery record. Any statement below that assigns Methods, exact capabilities, policy, bounds, projection dimensions, or requested authority to a stewardship package is superseded by [Canonical Persistent Domain Stewardship](../../cognitive_architecture/persistent_domain_stewardship.md).
...
Slice one CLOSED 2026-07-30, theory composition and load-path alignment, merged from rc-ignition-theory: the outcome-mapping injection widened to the installed set so the assembled ingestion actor binds the full installed vocabulary; `meld world init --theory-source` provisions a canonical theory package into the XDG theory root with identity checks before writes and byte idempotency; the CLI composes outcome-mapping and planning theory at assembly load, best-effort with truthful unresolved bindings on failure, giving `PlanningTheoryBinding` its first production constructor over the workflow task-path catalog; the selection vocabulary normalized to the shipped identities; and the docs freshness package now ships its curation rule, refresh method, available-action binding, and method realization. This discharges the evidence-mapping and planning composition findings, the load-path and selection-identity findings, and the mapping-set half of the vocabulary finding. Live validation through the served surface showed every composed coupling narrating per-tick waiting-on declarations and exposed one truthfulness defect fixed in the slice: the ingestion actor's empty-page return skipped the quiet-ledger declaration. Fresh review passed with one corrected blocking finding, a content-derived method identity that could escape the theory root. Non-blocking review residuals recorded for later disposition: the provisioning identity checks run per body rather than all before the first write, duplicate method identities across source files last-write-win, and methods without a Holds trigger dimension contribute silently to requested dimensions. The remaining stall is the anchor and identity-vocabulary semantics: belief assessment waits on `graph_anchor_absent` under the hardcoded analysis perspective, exactly the anchor-optional cold start and canonicalization findings.

Slice two CLOSED 2026-07-30, anchor-optional assessment and genesis belief motion, merged from rc-ignition-belief: whether initial assessment consumes a graph anchor is now family-declared theory — `BeliefFamilyConfig` gains a serde-defaulted `anchor_requirement`, Required keeps the existing semantics verbatim, and an Unanchored family assesses an unobserved subject to its prior-based revision with the source cursor window pinned to zero so first evidence re-dirties the key. The comparator accepts an empty evidence window when the caller supplies the configured belief key. BREAKING: the public ComparatorInput gains a required subject_key field and the family config content hash changes, forcing a one-time global reassessment on upgrade. The assembly derives the anchor perspective from the agent identity on the production frame-type convention instead of the hardcoded analysis literal. The shipped docs freshness family declares Unanchored and gains the unobserved-scope signal; the mapping set gains the genesis rule, so belief motion from the stage 4 genesis fact occurs with no new runtime code. The harness survey fixture freezes the pre-ignition family body verbatim, preserving the anchor stall as the validation specimen. Live validation over the served surface: genesis evidence promoted and committed, belief assessed to belief_work_ineligible with no anchor stall anywhere, curation proposed the goal, planning composed against it, and the eligibility walk's dead ends moved to the chartered dispatch-valve and publication findings. Fresh review passed; corrections applied in-lane were the breaking-change marker, the served-surface probe retargeted to the derived perspective, and a motion test pinning posterior 0.875 and confidence 0.125 from genesis. Non-blocking residuals recorded for later disposition: a crash between unanchored lease acquisition and commit leaves a dirty zero-window that suppresses initial selection silently until evidence heals it, wanting an empty-window clear or a waiting-on declaration; and factor weights cancel in per-event solo windows, so the genesis factor's low weight is inert alone — reconcile theory-author intent with comparator behavior before the convergence proof. Reconciliation escalated 2026-07-31 with a measured failed candidate: damping semantics break the satisfaction arc under the installed theory numbers, so comparator semantics, factor weights, and the satisfaction threshold are one coupled calibration; decision brief in [Flywheel Parity Workstream](flywheel_parity_workstream.md) slice five.

Slices three and four CLOSED 2026-07-30, publication path and dispatch valve, merged from rc-ignition-publication: the per-task publication outbox drains through the assembled `execution.publication` actor on every tick before its aggregate pass — bounded by the tick budget, appended under the bridge's deterministic record ids, sessioned by the composed partition — discharging the NAG-2 product-caller finding; both production dispatch invokers pass the composed session as the execution event context so per-task lifecycle events reach the ledger; `--enable-runtime` opens the dispatch valve by removing a runtime from the default-disabled set with a loud error for unknown ids; a resolved stewardship composition marks the provider available since the binding validated it, leaving reachability a retryable runtime concern; and the foreground run writes every assembly diagnostic through the account writer before the supervisor starts. Two blockers found live and fixed in-lane: planning narrated nothing when an attempted goal did not compose — it now declares no_applicable_method with bounded candidate dispositions or world_state_indeterminate with the missing terms — and the planner projection derived scope accessibility from anchors alone, so an unanchored family could never satisfy an Accessible precondition; the projection now follows the family anchor declaration, with the maintained scope declared accessible from the validated selection. Publication quiet and skip paths narrate: no_pending_publications only on an error-free confirmed tick, aggregate_run_not_terminal with the pending work units. Live validation ran the full arc end to end: genesis to evidence to prior-based revision to curated goal to composition to package handoff to dispatch preparing the docs writer run and invoking capability steps against the provider boundary, failing retryably on an unreachable endpoint with every coupling narrating truthfully. Fresh review passed; the corrected blocking finding was a quiet-tick declaration that could assert an unconfirmed empty outbox over an errored bridge. Recorded residuals: lifecycle event re-emission on resumed claims lacks idempotent record ids, a pre-existing mechanism newly reachable on the dispatch routes; the two planner projection methods should share a core; agent curation's world-state projection still derives accessibility from anchors and needs the family-declared treatment if a satisfaction rule ever conditions on Accessible; homogeneity-asserting account-stream parsers will meet heterogeneous lines when diagnostics are present. Full completion of the return arc awaits a reachable provider, which is the convergence proof's environment requirement.

FIRST FULL SEMANTIC TURN 2026-07-30, live evidence toward the convergence proof: with all four ignition slices integrated, a real provider behind an OpenAI-compatible lab gateway, and only product operations driven — init, world init with the shipped theory source, scan, and a foreground run with dispatch enabled — the flywheel completed one full turn with the harness attached. The genesis fact promoted to evidence and committed the prior-based revision at confidence 0.125; curation proposed the goal; planning composed and selected the package route; dispatch prepared the docs writer run and drove thirty-three bounded steps through seventeen real provider completions; README files materialized in both actionable folders with child finalization preceding parent preparation; per-task publications drained through the bridge and the aggregate published on terminality; ingestion mapped the published outcomes into evidence; the revision crossed the satisfaction threshold; satisfaction curation closed the goal through the owner-only mutation; planning returned to a truthful no-active-goals wait and the supervisor held quiescence across the remainder of the run. Every transition was observed through the served surface with no synthetic input anywhere. This is live evidence for guide steps one through twelve; the proof of record remains the deterministic root integration test the guide specifies — workflow-baseline comparison, drift wakeup, and re-eligibility under a fresh reviewer at high strength — and stays open.
...
- The runtime proof previously synthesized a placeholder artifact and success outcome; the synthesizing replay port was removed 2026-07-31 and its helper survives only as reopen-contract test support.
- Real writer artifacts and publication payloads are aligned with the world-model mapping input as of 2026-07-31: the aggregate payload carries the package-declared semantic yield the installed completed interpretation discriminates on.
...
9. Preserve canonical task outcome, per-folder publish results, artifact records, and the package-declared semantic yield summary intact inside the aggregate publication payload.
