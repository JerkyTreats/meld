...
Status: closed 2026-07-26, merged on `runtime-completion`. The serve domain landed the loopback `/v1` listener whose JSON bodies are the contract types verbatim, mapping one to one from the event authority contract (the subscription long-poll is the blocking watch) plus the report store reader and both walks (`8d3aa49`); the concurrent-status finding closed by the foreground run serving the substrate with a discovery advertisement and a lock-free CLI bypass through `describe_for_workspace` (`dccd270`); the three projections and delegation scope landed as substrate derivations (`7f743d0`); fresh review found three blocking findings — the phase-2 session-fencing residual served over reused roots, projections missing from the byte-consistency probes, and silent queue-depth saturation — all corrected (`cdc9c84`). The authoritative harness gate passed: the survey stall presents at all three altitudes from one session record, consumed by a separate process over the published URL, every projection citing the same durable action id and exact subject key; live and playback bodies are byte-identical across the probe set. This discharges the register's cross-process stall-presentation exit conjunct; the Goal-and-Belief prototype session remains the register's one open conjunct, owed with Strategy first-slice development. Residuals for phase four: the playback mount should reject appends over sealed roots; the append-endpoint reading of the read-only seam (canonical sensory port, not a mutation of runtime state) is recorded in the serve domain rustdoc; the composed-network lock retry deserves a typed error and no-retry read path; startup snapshots still embed cross-boot latest actions.
