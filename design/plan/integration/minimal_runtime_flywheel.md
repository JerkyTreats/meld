...
| `execution` | `own` | Goals, planning, lowering, task network contracts, task network store, task runtime bridge, and outcome records exist. | Glue is needed from curated goal command to active goal, from planning to task network, from publication outbox to spine, and from updated world state to satisfaction. | `partial` | [goals](../../../crates/meld-execution/src/goals.rs), [planning](../../../crates/meld-execution/src/planning.rs), [lowering](../../../crates/meld-execution/src/planning/lowering.rs), [task network](../../../crates/meld-execution/src/task_network.rs), [task bridge test](../../../crates/meld-execution/tests/task_network_execution_bridge.rs) | none | Implement the minimal flywheel runtime and publication worker. |
...
| `lib` | `adapter` | Public crate surface exports current domains but does not expose a single flywheel runtime. | Public export is only needed if the first proof is hosted outside internal tests. | `partial` | [lib root](../../../src/lib.rs) | none | Export the orchestrator only after its owner is chosen. |
...
| 5 | `world_state` agent | `execution` goals | `AgentGoalCommand` to `AddGoalCommand` | No adapter found | Validate, activate, store, and dedupe curated goal commands | missing | `not started` |
...
| 9 | `execution` task network | `events` | pending `Publication` or task event envelopes | Task runtime can emit envelopes, task network creates publication outbox | Publish pending task network outcomes and mark publication state | [task runtime](../../../crates/meld-execution/src/task/runtime.rs), [outcome contracts](../../../crates/meld-execution/src/task_network/outcome.rs) | `partial` |
...
| 11 | `world_state` planner | `execution` goals | post outcome `WorldState` plus active goal | Goal store can mark satisfied | Evaluate after outcome replay and call `SatisfyGoalCommand` | [goal store tests](../../../crates/meld-execution/tests/goals.rs) | `not started` |
...
| publication worker | `execution` and `events` | missing | event spine plus task network store | pending publications | appended event facts and marked publications | `not started` |
| satisfaction reviewer | `execution` and `world_state` | missing | goal store plus world state stores | active goals and updated projection | satisfied or unsatisfied goal records | `not started` |
...
| belief views | `world_state` | `BeliefStore` | same, assembled in runtime | reassess subject after new evidence | outcome evidence source not chosen |
| agent decisions | `world_state` | `AgentStore` | same, assembled in runtime | dedupe curation by decision key and subscription cursor | not assembled in flywheel |
| execution goals | `execution` | `PersistentGoalSetStore` | same, assembled in runtime | reopen active and satisfied lifecycle state | curation adapter missing |
...
The second gap is the curation to execution goal handoff. `AgentCuration` emits `AgentGoalCommand`, while `PersistentGoalSetStore` accepts execution goal commands. The adapter between those contracts is missing.

The third gap is publication. Task runtime can emit execution task envelopes when called with event context, and task network can create publication outbox records, but the flywheel lacks one selected publication path that appends outcome facts into the event spine and marks publication state.

The fourth gap is the post outcome world model update. `WorldStateReducer` consumes selected execution events, but the docs freshness belief update must be tied to the chosen docs writer outcome fact.

The final gap is satisfaction. Goal stores support `SatisfyGoalCommand`, and the language runtime can evaluate satisfaction, but no runtime reviewer evaluates the updated projection and mutates the goal lifecycle.
...
| Implement `AgentGoalCommand` to execution `AddGoalCommand` adapter | `execution` | `world_state` | [agent curation](../../../crates/meld-world-model/src/agent/curation.rs), [goals](../../../crates/meld-execution/src/goals.rs) | `cargo test -p meld-execution --test goals` | `P0` |
...
| Add task outcome publication bridge | `execution` | `events` | [task network outcome](../../../crates/meld-execution/src/task_network/outcome.rs), [event store](../../../crates/meld-events/src/events/store.rs) | `cargo test -p meld-execution --test task_network_execution_bridge` | `P0` |
| Connect docs writer outcome to docs freshness belief update | `world_state` | `events`, `execution` | [world state reducer](../../../crates/meld-world-model/src/world_state/reducer.rs), [planner projection](../../../crates/meld-world-model/src/planner/projection.rs) | `cargo test -p meld-world-model --test planner` | `P0` |
| Add satisfaction reviewer over active goals and updated projection | `execution` | `world_state` | [goal store test](../../../crates/meld-execution/tests/goals.rs), [evaluation loop](../../../crates/meld-lang/tests/evaluation_loop.rs) | `cargo test -p meld-execution --test goals` | `P1` |
