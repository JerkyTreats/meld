...
The durable runtime assembly shape is defined in [Durable Runtime First Slice](durable_runtime_first_slice.md). This document defines the target loop. The durable slice defines how to wire domain runtimes without centralizing domain semantics.
...
| `error` | `observe` | Store, API, execution, and world model errors already have stable mappings through domain error types. | The supervisor needs to propagate or summarize failures without changing domain truth. | `partial` | [execution ports](../../../crates/meld-execution/src/execution/ports.rs), [task network store error tests](../../../crates/meld-execution/tests/task_network_store.rs) | none | Define the failure outcome contract for the end to end flywheel test. |
...
| `lib` | `adapter` | Public crate surface exports current domains but does not expose a single flywheel runtime. | Public export is only needed if the first proof is driven outside internal tests. | `partial` | [lib root](../../../src/lib.rs) | none | Export assembly only after its owner is chosen. |
...
| 8 | `execution` task network | `task` runtime | dispatch `Claim`, materialized init payload, `TaskExecutor` | Bridge test claims, executes, records outcome | Same path inside execution runtime | [task network bridge test](../../../crates/meld-execution/tests/task_network_execution_bridge.rs) | `complete` |
...
Satisfaction review is implemented as an agent curation boundary plus root integration adapter. The remaining work is to wire it after outcome evidence ingestion and planner projection refresh without making root the satisfaction driver.

The durable assembly must follow [Durable Runtime First Slice](durable_runtime_first_slice.md): one product process may run every domain runtime, but each cross-domain transition must be a direct handoff through domain stores, event append, or command APIs.
