...
Measured underneath, pinned by the incremental staleness contract: publication writes README.md files back into the tree node identity is keyed on, so every published folder re-identifies on the next scan and an unforced rerun regenerates everything, while with published artifacts absent one mutation regenerates precisely its three-folder ancestor chain. This is not an execution-layer blocker to fix with identity surgery — the tree re-identifying after publication is the world truthfully changing. **The regeneration loop is cut by belief, not by identity mechanics**: the published outcome carries its yield, evidence moves the freshness score, and a passing score satisfies the goal so no further dispatch occurs. That epistemic cut is parity slice four. The measured full-scope regeneration cost when a dispatch does follow a publication turn stands as an efficiency residual only. Disposition: zero-work refusal and completion guards landed in slice three; the loop cut is slice four; per-turn regeneration cost recorded as residual.
