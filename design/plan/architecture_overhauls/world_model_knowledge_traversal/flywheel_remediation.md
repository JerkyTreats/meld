...
| V04, V17 | `extract_claims` in [claim validation](../../../../owners/docs/src/docs/claim_validation.rs), [observation](../../../../owners/docs/src/docs/observation.rs), and [claim observation](../../../../owners/docs/src/docs/claim_observation.rs) |
| V05 | [Security policy](../../../../owners/dependency-security/src/dependency_security/policy.rs), [assessment](../../../../owners/dependency-security/src/dependency_security/assessment.rs), and [verification](../../../../owners/dependency-security/src/dependency_security/verification.rs) |
...
| V16, V18 | [Activation declaration](../../../../src/config/stewardship/activation.rs), [Docs runtime](../../../../owners/docs/src/docs/runtime.rs), [Security runtime](../../../../owners/dependency-security/src/dependency_security/runtime.rs), and [Context API](../../../../src/api.rs) |
...
### R4 Native Observation Ownership — Accepted Increment

The user requested `Continue r4` from `d158f939`. The next coherent source boundary moves observation actor construction and dispatch behind the existing native lifecycle contract. Docs and Security publish observation factories, and core dispatches bounded ticks and lifecycle requests through one `NativeObservationOwner` interface. Their concrete actors become private to their domains. The Docs provider judgment adapter moves out of runtime ports into the Docs owner; the CLI supplies the generic provider completion port. Production Docs-specific judge-binding entrypoints are removed, and native callers migrate together.

This extends the existing owner contract and replaces concrete actor dispatch, without a new scheduler, store, serialized protocol or semantic authority. The affected behavior surfaces are Runtime lifecycle and assembly, Docs and Security native observation factories, and the CLI provider adapter. Provider, Events, Curation, Agent, Strategy and Execution retain their current contracts. Durable binding identity and publication interpretation remain unchanged. The existing compiled factory selections remain explicit R4 extraction work; this increment does not declare external hosting implemented. Rust API callers replace `bind_production_docs_claim_judge` with `bind_observation_provider`. Direct `bind_docs_claim_judge` injection is retained only as a private unit-test fixture hook, not a production API. The breaking-change commit must disclose this migration.

Validation is bounded to existing native observation, provider-return recovery, shared-work and CLI progression coverage plus all-target compilation and lint. The previous real-Qwen proof remains semantic regression evidence; no new model behavior is introduced by passing the same provider through an owner-facing port. The observer actors' concrete constructors and old production judge-binding API must have no callers outside their owning domains in the completed source. Read-only observation inspection uses the owner binding's current-revision query without constructing another actor. No new persisted format or migration is required.


The bounded increment passed a fresh neutral logical and style review with no source blocker. Eighteen focused tests passed: ten Docs owner tests, one Security owner test, four Docs failed-work recovery tests, one shared-request progression test, one repeated native Security mitigation test with independent verification, and one prepared-product CLI test. The original Security filter matched zero tests and was excluded from evidence; the corrected invocation passed. All-target Clippy passed with the same 79 existing `result_large_err` sites, and formatting and whitespace checks passed.

Evidence is retained at `~/.local/state/meld-flywheel-remediation/r4-observation-gate-20260909T124746Z`. Candidate identity is `d158f939` plus the captured source manifest with SHA-256 `f8b0ab6c38801516d0d63301fad142c014d015af40c5da9e080a88aaeaabb253`; source remained unchanged throughout validation. The evidence directory contains commands, logs, source and artifact manifests, candidate diff and the bounded neutral review result.

If applied, this commit makes native observation dispatch owner-neutral, confines concrete actor construction to Docs and Security, and moves Docs provider judgment into its owner. It retires the production Docs-specific judge-binding methods and root judgment adapter. This accepts only this boundary increment; external hosting, compiled factory selection removal, independently scoped assignments and whole R4 acceptance remain open.


### R4 Completion — In Progress

The user explicitly requested `complete r4` from `a01c9154`, extending implementation authority through the whole R4 outcome. The accepted boundary design and issue dispositions above remain authoritative. Work proceeds through coherent commit candidates, but an intermediate bridge, source move or test fixture does not close R4. No push is authorized by this request.

The affected write surface is Runtime owner connection, preparation and lifecycle composition; Theory route and exact resource selection; initialization and assignment selection; and the Docs and Security semantic packages. Existing Events, provider completion, capability invocation, Curation, Agent and Strategy contracts are reused. Capability results and native owner snapshots gain serialization for intact transport. No second Event writer, product scheduler, planner or Task admission authority is introduced. Causation/regime, PDS language expansion and unrelated R5 policy changes remain deferred.

Native observation and Task invocation share one serial owner connection. The owner returns its existing lifecycle receipts and capability products. Parent callback grants distinguish observation, exact selected capability invocation and read-only recovery. A bounded neutral boundary review identified lost implementation selection, owner-wide invocation publication grants, and excessive rights on lifecycle queries. The working bridge now carries exact contract and implementation selection on invocation and recovery, scopes invocation route grants to that selection, and supplies read-only callbacks for snapshots and wake queries. These are source corrections under review, not an R4 acceptance verdict.

Docs and Security source now lives in `owners/docs` and `owners/dependency-security`. Core no longer exports these product modules or constructs their semantic observation actors, policies or capability contributors. The package executables depend on native Meld contracts; Meld has no production dependency on either semantic package. Root installation selects operator-configured executable descriptions, routes immutable bodies through the canonical Theory router and carries owner-issued resources into exact capability preparation. Runtime observation and capability invocation share each selected owner connection. The installation inventory remains broader than the selected runtime owners.

The current focused process evidence passes external Docs installation, native observation publication, capability invocation, predecessor/successor preparation overlap, release and reopen. Separate ordinary initialization fixtures pass installation and exact reopened capability preparation for Docs and Security through `ProductRuntimeAssembly` and the normal world initialization pipeline. These are real child-process tests, but they do not prove the full external flywheel or unchanged-core revision acceptance. Latest focused logs are `/tmp/meld-r4-docs-hosting-final.log` and `/tmp/meld-r4-ordinary-security.log`. Both Docs hosting cases and the Security initialization case pass. The workspace binary check passes without warnings in `/tmp/meld-r4-binaries-check.log`. Three startup integration cases pass, including the ordinary executable nonce path, in `/tmp/meld-r4-startup-integration.log`. Qwen has not been rerun on this candidate.

The bounded composition review found three material integration defects: the absent Security invocation-return publication grant, activation of configured but unselected owners, and provider binding that excluded capability-only owners. Source corrections add the exact receipt route, select operational owners from installed revisions and participant plans, and bind providers to all selected runtime instances. No final gate verdict is claimed. Historical reconstruction now opens the retained executable selection from the predecessor activation while sharing the existing core stores; immutable owner-store access remains serialized per command.
