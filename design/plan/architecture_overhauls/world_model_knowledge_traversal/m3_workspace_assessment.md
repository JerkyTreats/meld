...
For an exercised workspace journey, evidence must show: no workspace requirement for Startup; a workspace-bound product can scan and publish through the canonical path; status distinguishes retained observation from a fresh filesystem check; ignore changes have truthful effects; live ownership blocks competing writers; interruption preserves index/publication distinctions; and retired commands no longer reach old writers. Filesystem-watch parity is required only if continuous observation is retained as a candidate claim, and must not be inferred from the passive participant.
