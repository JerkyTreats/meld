...
```mermaid
flowchart TD
    Repo[Gmail Operator checkout and changes] --> Sensor[External repository sensor and Tree-sitter extraction]
    Sensor --> Events[Canonical Meld Events]
    Events --> Admission[Owner admission and explicit coverage]
    Admission --> Knowledge[Native shared representation and queries]
    Knowledge --> Claims[Later domain claims curation]
    Claims --> Consumers[Later README and other domain agents]
    Harness[External harness] --> Repo
    Harness --> Events
    Events --> Evidence[Public command evidence]
    Knowledge --> Evidence
    Evidence --> Author[Revise producer domain theory or a demonstrated runtime gap]
    Author --> Sensor
```
