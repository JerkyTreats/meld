...
| Reconciliation requests | agent request-status KEY | agent request with stable key | Native completion evidence | Request storage/intake exists for supported maintained intentions. Admission-epoch Startup rejects explicit requests by design. Exercise lookup and lost-response handling on an eligible product. |
...
The native runtime, Agent and Event surfaces are now exercised through the runtime as recorded in the [closeout](m3_01_installed_startup.md#native-command-control-closeout). Remaining World, provider and workspace proposals are unqualified exploration prompts. The native shutdown identity made a separate control-request store unnecessary; retained shutdown lookup and repeat-safe stop use that existing authority. M3 labels group backlog concerns, not implementation dependencies. Use the next observed gap to choose which operation to exercise and update this assessment when the evidence changes.
