...
The user has now authorized branch `feat/readme-pds-event-claims` and implementation of the [Event-authored README candidate](readme_event_claims_candidate.md). Exact public Event submission is implemented and proved. The independent domain has closed a three-obligation native root repair loop and a source and prose maintenance loop for a leaf README on Luna-low. The wider root qualification is still active. The record separates generic runtime transport from external domain representation and retains failed candidates alongside successful bounded proofs.
