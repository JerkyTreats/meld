...
Date: 2026-09-11. Mode: native command-control delivery and initial README PDS qualification complete locally. Startup, managed control, deterministic Docs and the bounded Luna-low root-and-leaf README candidate pass their external journeys. Corpus rollout, broader products and release qualification remain separate.
...
The independent README domain now has a bounded self-writing prototype, recorded in the [initial PDS checkpoint](readme_pds.md#initial-independent-pds-checkpoint). It installs and runs through existing native contracts without Meld source changes, repairs source and prose changes, and reuses unchanged judgments on restart. The full Meld root README production outcome remains open; the prototype does not establish corpus-wide maintenance or release readiness.

The first held-out maintenance challenge on Meld's actual root README has now concluded with a false editorial acceptance. The model received the changed command syntax but accepted the obsolete example. [The result](readme_pds.md#held-out-meld-root-maintenance-result) leaves the production README outcome unaccepted and identifies specific claim assessment as the next domain gap. Native packaging, currentness and persistence continued to work; no core architectural circuit breaker tripped.

The user subsequently authorized pushing branch `feat/meld-3-candidate` at `fefe7b89` and constrained diagnosis to harness and Meld commands. That push completed. [Eight fresh observation probes](readme_pds.md#judgment-isolation-through-the-command-surface) isolate false acceptance and invented evidence in the provider's semantic grading, while the published owner verdict faithfully matches the response. Neither source reduction nor document reduction alone fixes it; the corrected focused control also contains unsupported criticism. The harness now captures those decisions through public Events and keeps observation success separate from product acceptance. No runtime or domain judge correction was made, and README production qualification remains open.

The user then authorized a [direct Codex provider comparison](readme_pds.md#direct-codex-provider-comparison). The generic provider adapter and public execution evidence now work through existing native contracts, with the independent owner and standard unchanged. Luna-low identified the stale command once, but the final candidate accepted both stale and corrected controls. Provider integration is accepted at this bounded scope; reliable root README grading and production qualification remain open. No new core semantic authority or T3Code server change was needed. Harness checkpoint `60e0edc` records stop-on-failure probes, and the slice retains native results, failure corrections and the parallel-test caveat.

The subsequent [Terra-low comparison](readme_pds.md#terra-comparison-of-the-frozen-judgments) identifies the injected stale flag, drops that complaint after correction and consistently identifies a second source-supported issue concerning retired context generation. An adaptive control correcting both issues is accepted. The original positive control was not independently clean. These three observations support trying the existing native repair loop with Terra; they do not establish repeatable grading or root README production readiness. The judge, package, runtime and harness implementations remain unchanged, with all investigation captured through public commands.

The [local Qwen3.8 comparison](readme_pds.md#local-qwen38-comparison) accepts all three controls, including the known stale flag, and fails the grading bar with thinking disabled. One initial call exposes an owner deadline shorter than inference, followed by failed-owner shutdown; the failure remains recorded. Raising the external binding deadline permits normal capture and shutdown for the completed controls without implementation changes. Input and schema parity with Terra is verified. Terra retains the stronger bounded judgment result; production README qualification remains open.

The [Qwen3.8 thinking test](readme_pds.md#qwen38-thinking-and-long-call-lifecycle-failure) times out twice at fifteen minutes without a judgment. The retry exposes a runtime lifecycle flaw: the provider deadline coincides with lease expiry, and shutdown fails with a stale lease after the blocking call. Longer-call qualification stops pending correction of lease liveness and failure drain; this result cannot be scored as a semantic rejection or acceptance. Harness checkpoint `e44042f` fixes final report loss after a nonzero native stop, with seventeen tests passing. No Meld runtime or domain implementation changed.

The [Luna effort comparison](readme_pds.md#luna-reasoning-effort-and-the-bounded-correctness-bar) finds high is the lowest tested Luna effort reproducing all original two-defect outcomes across repeated controls; refreshed Terra-low reproduces them faster. Low invents a bootstrap prerequisite disproved by 22 public first-use checks, and medium drops an unchanged limitation. Broader audit concerns still vary, and an additional control is needed to clarify supported ownership and isolation issues. Both high and Terra accept that adaptive control. Terra-low is not established as a minimum model tier, and reliable whole-README correctness remains unqualified. The study is complete without runtime or domain implementation changes; the long-call lifecycle flaw remains open.


Branch `feat/readme-pds-event-claims` now contains the [qualified initial README candidate](readme_event_claims_candidate.md). Meld wrote the actual root README through its native PDS. The final nineteen-obligation program passes flattened-root reconstruction, live Event revisions, exact source-change repair, uncertainty, native confirmation and zero-call restart reuse on Luna-low. The final leaf package passes authorship, source and prose maintenance, missing-source withholding and restoration. Earlier failed judgments above remain historical evidence; the successor changes domain representation and selected CPU verification. No new core semantic authority was needed. Automatic corpus management and release qualification remain open.
