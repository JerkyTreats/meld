...
```mermaid
flowchart TD
    Files[Observed project files] --> CS[Codebase Semantics WAD]
    Authored[Authored README intent and knowledge] --> EV[Events]
    CS --> EV
    EV --> WM[Owner admission and shared Graph]
    WM --> EB[Evidence and Belief revisions]
    D[Agent Directive] --> AJ[Agent judges divergence or uncertainty]
    EB --> AJ
    AJ --> G[Ground Goal]
    G --> S[Native Strategy decomposition]
    WM --> CUT[Immutable relevant planner cut]
    EB --> CUT
    CUT --> S
    Methods[WAD methods and capability contracts] --> S
    S --> P[Plan with causal and decomposition lineage]
    P --> AU[Agent product authorization]
    AU --> CU[Curation epistemic operations]
    AU --> EX[Execution Task Network]
    EX --> Effects[Bounded assessment drafting and publication]
    Effects --> Observation[Observe actual result]
    Observation --> EV
    CU --> EV
    P --> H[Public explanation and harness measurements]
    EV --> H
    H --> Author[Improve domain knowledge or runtime from failures]
    Author --> Authored
    Author --> Methods
```
